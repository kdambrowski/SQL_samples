-- Write a query to retrieve all passengers who live in Tokyo or London and had a flight between 2020-01-01 and 2021-10-01.
...
WHERE flight.departure_date BETWEEN '2020-01-01T00:00:00.000' AND '2021-10-01T23:59:59.999'
    AND a.city IN ('Tokyo', 'London');

-- Write a query to count the number of flights for each airplane from 2020-01-01 until today. 
SELECT s.id AS airplane_number, 
    COUNT(s.id) AS number_of_flights_from_2020_01_01_to_today 
FROM flight
    JOIN airplane AS s ON s.id = flight.airplane_id1
WHERE flight.departure_date BETWEEN '2020-01-01' AND NOW()
GROUP BY s.id;
...
SELECT s.id AS airplane_id, 
    COUNT(DISTINCT flight.flight_number) AS number_of_flights_from_2020_01_01_to_today, 
...
-- Write a query to retrieve, for each passenger, their 4 most recent flights with a breakdown by airplane manufacturer.
...
-- each month in the period from 2020-01-01 to 2023-03-28. Additionally, add the cumulative sum of passengers
...
WHERE
   flight.departure_date BETWEEN '2020-01-01T00:00:00.000' AND '2023-03-28T23:59:59.999'
GROUP BY
    airplane.id,
    airplane.manufacturer,
    airplane.type,
